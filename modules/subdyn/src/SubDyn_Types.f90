...
subroutine SD_PackAddedMassType(RF, Indata)
   type(RegFile), intent(inout) :: RF
...
subroutine SD_UnPackAddedMassType(RF, OutData)
   type(RegFile), intent(inout)    :: RF
...
subroutine SD_DestroyElemPropType(ElemPropTypeData, ErrStat, ErrMsg)
   ...
   ErrStat = ErrID_None
   ErrMsg  = ''
   call SD_DestroyAddedMassType(ElemPropTypeData%AddedMass, ErrStat2, ErrMsg2)
   call SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
end subroutine
...
subroutine SD_PackElemPropType(RF, Indata)
   ...
   call SD_PackAddedMassType(RF, InData%AddedMass) 
   if (RegCheckErr(RF, RoutineName)) return
end subroutine
...
subroutine SD_UnPackElemPropType(RF, OutData)
   ...
   call SD_UnpackAddedMassType(RF, OutData%AddedMass) ! AddedMass 
end subroutine
...
subroutine SD_CopyInitType(SrcInitTypeData, DstInitTypeData, CtrlCode, ErrStat, ErrMsg)
   ...
   if (allocated(SrcInitTypeData%MG)) then
      LB(1:2) = lbound(SrcInitTypeData%MG)
      UB(1:2) = ubound(SrcInitTypeData%MG)
      if (.not. allocated(DstInitTypeData%MG)) then
         allocate(DstInitTypeData%MG(LB(1):UB(1),LB(2):UB(2)), stat=ErrStat2)
         if (ErrStat2 /= 0) then
...
subroutine SD_PackInitType(RF, Indata)
   ...
   call RegPackAlloc(RF, InData%MG)
...
subroutine SD_UnPackInitType(RF, OutData)
   ...
   call RegUnpackAlloc(RF, OutData%MG); if (RegCheckErr(RF, RoutineName)) return
...
subroutine SD_CopyParam(SrcParamData, DstParamData, CtrlCode, ErrStat, ErrMsg)
   ...
   if (allocated(SrcParamData%NodeAddedMass)) then
      LB(1:1) = lbound(SrcParamData%NodeAddedMass)
      UB(1:1) = ubound(SrcParamData%NodeAddedMass)
      if (.not. allocated(DstParamData%NodeAddedMass)) then
         allocate(DstParamData%NodeAddedMass(LB(1):UB(1)), stat=ErrStat2)
         if (ErrStat2 /= 0) then
...
subroutine SD_DestroyParam(ParamData, ErrStat, ErrMsg)
   ...
   if (allocated(ParamData%NodeAddedMass)) then
      LB(1:1) = lbound(ParamData%NodeAddedMass)
      UB(1:1) = ubound(ParamData%NodeAddedMass)
      do i1 = LB(1), UB(1)
         call SD_DestroyAddedMassType(ParamData%NodeAddedMass(i1), ErrStat2, ErrMsg2)
         call SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
      end do
...
subroutine SD_PackParam(RF, Indata)
   ...
   call RegPack(RF, allocated(InData%NodeAddedMass))
   if (allocated(InData%NodeAddedMass)) then
      call RegPackBounds(RF, 1, lbound(InData%NodeAddedMass), ubound(InData%NodeAddedMass))
      LB(1:1) = lbound(InData%NodeAddedMass)
      UB(1:1) = ubound(InData%NodeAddedMass)
      do i1 = LB(1), UB(1)
         call SD_PackAddedMassType(RF, InData%NodeAddedMass(i1)) 
      end do
   end if
   if (RegCheckErr(RF, RoutineName)) return
end subroutine
...
subroutine SD_UnPackParam(RF, OutData)
   ...
   if (IsAllocAssoc) then
      call RegUnpackBounds(RF, 1, LB, UB); if (RegCheckErr(RF, RoutineName)) return
      allocate(OutData%NodeAddedMass(LB(1):UB(1)),stat=stat)
      if (stat /= 0) then 
         call SetErrStat(ErrID_Fatal, 'Error allocating OutData%NodeAddedMass.', RF%ErrStat, RF%ErrMsg, RoutineName)
         return
      end if
      do i1 = LB(1), UB(1)
         call SD_UnpackAddedMassType(RF, OutData%NodeAddedMass(i1)) ! NodeAddedMass 
      end do
   end if
end subroutine
